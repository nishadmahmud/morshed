...
const Page = ({ params }) => {
  ...
  useEffect(() => {
    if(sortBy === "low-to-high" && sortBy){
      setFilteredItems([...filteredItems].sort((a,b) => a.retails_price - b.retails_price));
    } else if(sortBy === "high-to-low" && sortBy){
      setFilteredItems([...filteredItems].sort((a,b) => b.retails_price - a.retails_price));
    } else if(sortBy === "a-z"){
  ...
  return (
    <div className="md:px-12 p-5 relative">
      ...
      <div 
        className={`fixed inset-0 bg-black bg-opacity-50 z-40 transition-opacity ${isFilterOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`}
        onClick={() => setIsFilterOpen(false)}
      ></div>
      <div 
        className={`fixed top-0 left-0 h-full w-64 bg-white shadow-lg z-50 p-5 overflow-y-auto transform transition-transform ${isFilterOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <button 
      ...
      <div className="grid md:grid-cols-4 lg:grid-cols-5 lg:pt-6 mt-1 gap-5">
        ...
        <div className="md:col-span-3 lg:col-span-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            {isLoading ? <Loader /> : filteredItems && filteredItems.length > 0 ?  
