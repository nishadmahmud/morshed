            ...
            <div className='w-full bg-[#000000] text-white z-40 py-2.5 px-5 hidden items-center text-center gap-16 roboto text-sm md:hidden lg:flex justify-between'>
                ...
                <div className='flex items-center justify-between'>
                 ...
                 <House color='gray' size={16}></House>
                    ...
                    {
                       data?.data.length && data?.data.slice(0,7).map((item,idx) => {
                            return <Link key={idx}  href={`/category/${encodeURIComponent(item?.category_id)}?category=${encodeURIComponent(item?.name)}&total=${encodeURIComponent(item?.product_count)}`} className={`text-[#ffffff] text-md text-nowrap font-medium flex items-center gap-1 hover:text-[#c03b2c] transition-all ease-in-out  ${idx === 0 ? 'pl-1' : ''}`}>
  ...
  >
    ...
    Blog
            ...
            <div className="flex justify-around items-center py-2">
                <Link href="/" className="flex flex-col items-center text-sm hover:text-white text-[#c03b2c]">
                    <House className="text-2xl" />
                    <span className='text-white'>Home</span>
                </Link>
                
                <Link href='/offer' className="flex flex-col items-center text-sm hover:text-white text-[#c03b2c]">
                ...
                {loggedInUser ? (
                        <Link href='/profileDashboard' className='flex flex-col items-center text-sm text-[#c03b2c] hover:text-white'>
                            <Image unoptimized className='border-2 p-0.5 border-[#c03b2c] rounded-full' src={loginLogo} alt='navLogo' width={28} height={28} />
                            <span className='text-white'>Account</span>
                        </Link>
                    ) : (
                        <div onClick={() => setIsLoginModal(true)} className='flex flex-col items-center text-sm text-[#c03b2c] hover:text-white cursor-pointer'>
